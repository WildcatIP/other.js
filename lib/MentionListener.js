...
const identityActions = ['default', 'go', 'whisper']
const channelActions = ['default', 'go']
...
class MentionListener extends Listener {
  /**
   * @callback MentionListener.onMention
  ...
  /**
   * @param {MentionListener.onMention} on - Called when a mention is typed by
   *     the user.
   */
  constructor({on}) {
    super()
  ...
  onSetStagedMessage(message) {
    ...
    if (partialMatch) {
      ...
      return results.then((results) => ({
        chatCompletions: results.map((c) => Object.assign(c, {
          actions: c.isIdentity ? identityActions : channelActions,
        })),
      }))
    }
    ...
    return results.then((results) => {
      ...
      const result = results.find((r) => r.name.toLowerCase() === name)
