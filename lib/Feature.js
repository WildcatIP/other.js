import UserAgent, {ACTIVATE_CHAT_COMPLETE_RESULT, ACTIVATE_MESSAGE_ACTION, NAVIGATE, SET_CHAT_COMPLETE_RESULTS, SET_MESSAGE_ACTIONS, SET_SELECTED_MESSAGES, SET_STAGED_MESSAGE, UPDATE_STAGED_MESSAGE} from './UserAgent'
...
class Feature extends FeatureMetadata {
  ...
  _startListeningIfNeeded() {
    ...
    userAgent.on(ACTIVATE_CHAT_COMPLETE_RESULT, (event) => {
      ...
      if (navigateResult) {
        ...
        userAgent.emit(NAVIGATE, {replyTag: tag, to})
        return
      }
      const stagedMessageResults = results.filter((r) => r && r.stagedMessage)
      if (!stagedMessageResults.length) return
      userAgent.emit(UPDATE_STAGED_MESSAGE, {replyTag: tag, message: stagedMessageResults[0].stagedMessage})
    ...
    userAgent.on(SET_STAGED_MESSAGE, (event) => {
      ...
      Promise.all(this._listeners.map((l) => l.onSetStagedMessage(message))).then((results) => {
        ...
        if (chatCompletions.length) {
          ...
          userAgent.emit(SET_CHAT_COMPLETE_RESULTS, {replyTag: tag, layout, results})
        }
        ...
        if (stagedMessages.length) {
          ...
          userAgent.emit(UPDATE_STAGED_MESSAGE, {replyTag: tag, message})
        }
      })
    })
  }
  ...
  _startProvidingMessageActionsIfNeeded() {
    ...
    userAgent.on(SET_SELECTED_MESSAGES, (event) => {
      ...
      userAgent.removeAllListeners(ACTIVATE_MESSAGE_ACTION)
      userAgent.on(ACTIVATE_MESSAGE_ACTION, (event) => {
        const {action, messages} = event
        const matchedAction = actions.find((a) => a.label === action)
        if (!matchedAction) return
        matchedAction.on({messages})
      })
      userAgent.emit(SET_MESSAGE_ACTIONS, {
