...
class Feature {
  ...
  listen({to, on}) {
    ...
    if (to === 'message') {
      console.assert(this._chatternet)
    ...
    if (!this._userAgent) {
      ...
      this._userAgent.on(Events.ACTIVATE_CHAT_COMPLETE_RESULT, event => {
        ...
        const results = this._listeners.map(l => l.onActivateChatCompleteResult(action, result, message)).filter(r => r && r.stagedMessage)
        if (!results.length) return
        // TODO: Accept the first results. Need to figure out how to handle conficts
        this._userAgent.emit(Events.UPDATE_STAGED_MESSAGE, {replyTag: tag, message: results[0].stagedMessage})
