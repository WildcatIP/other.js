 ...
 */
export default class ChatternetRestClient {
  constructor(extraFetchHeaders, onFetchError) {
  ...
  fetchChannelMessages(channelId, since) {
    return this._fetchJSON('GET', `/channel/${channelId}/events?since=${encodeURIComponent(since)}`)
  }
  ...
  _fetchJSON(method, endpoint, requestJSON) {
    ...
    return global.fetch((process.env.COURIER_HOST || '') + endpoint, request).then(checkStatus).catch(this.onFetchError).then(parseJSON)
  }
}
