...
var RPS = {
  ...
  rootRef: feature.data.child( 'games' ),
  ...
  startGame: gameRef => {
    ...
    let actionPickers = [game.challenger, game.challengee].map( user => ({
        ...
        whoCanSee: user,
        didSelect: selected => historyRef.append({ by: context.user, action: selected.action })
      })
    )

    historyRef.on( 'ChildAdded', playerActionRef => RPS.onPlayerAction(playerActionRef, gameRef) )
  ...
  onPlayerAction: ( playerActionRef, gameRef ) => {

    let game = gameRef.value(),
        playerAction = playerActionRef.value(),
...
RPS.rootRef.on( 'ChildAdded', gameRef => RPS.startGame( gameRef ) )

rpsCommand.on( 'didAction', selected => {

  return RPS.rootRef.append({
